...
- 모든 원시(raw) 조회는 15초 TTL로 캐싱하여 API 호출 빈도를 제한한다 (Rate Limit 방지).
- 원시 조회 함수 자체가 예외를 삼키고 {"ok": bool, ...} 형태로 반환하므로, 실패도 함께
  캐싱되어 15초 동안은 재시도하지 않는다 (장애 시 API를 계속 두드리지 않도록 함).
...
def _raw_stock_daily_history(code: str) -> dict:
    try:
        closes = kis_api.get_domestic_stock_daily_history(code, target_days=30)
        kis_api.throttle()
        return {"ok": True, "data": {"closes": closes}}
    except Exception as e:
        return {"ok": False, "error": str(e)}
